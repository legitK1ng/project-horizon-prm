...
test.describe('Phase 3 Features', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.route('**/*', async route => {
            ...
            // Allow local assets (including lazy-loaded .js chunks)
            if (url.includes('localhost') || url.includes('127.0.0.1') || url.includes('vite') || url.includes('.tsx') || url.includes('.ts') || url.includes('.css') || url.includes('.js')) {
    ...
    test('should open Edit Contact modal', async ({ page }) => {
        // Navigate to Call Logs and wait for lazy-loaded view
        ...
        // Hover over the contact name to trigger the HoverCard
        await brandon.hover();

        // Wait for HoverCard content — it renders as an inline button with phone number
        const hoverInfo = page.locator('text=+15550102030');
        await expect(hoverInfo).toBeVisible({ timeout: 5000 });
    });
    ...
    test('should navigate to Processing Lab and analyze text', async ({ page }) => {
        await page.click('button:has-text("Processing Lab")');
        await page.waitForSelector('textarea', { timeout: 10000 });
        const textarea = page.locator('textarea');
        await textarea.fill('Testing Lab Analysis.');
        await page.click('button:has-text("Analyze Text")');
        // Wait for the analysis results section to appear
        await expect(page.locator('h3:has-text("Analysis Results")')).toBeVisible({ timeout: 10000 });
    });
