...
import { useNudges } from '@/hooks/useHorizonData';
...
const NudgeShelf: React.FC<NudgeShelfProps> = ({ onContactClick }) => {
  const { data: nudges, isLoading, isError, refetch } = useNudges();
  const isRefreshing = false;
  const scrollRef = useRef<HTMLDivElement>(null);

  // FIX: refreshHealth expects a contactId — use refetch() for a full nudge list refresh
  ...
  if (isError || !nudges || nudges.length === 0) {
     return (
       <div className="glass p-8 rounded-3xl border border-slate-200/50 dark:border-slate-800/50 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">Relationships are optimized. Check back later for new nudges.</p>
          <button 
          ...
          >
            ...
            {isLoading ? 'Analyzing...' : 'Force Analysis'}
          </button>
       </div>
     );
  }
